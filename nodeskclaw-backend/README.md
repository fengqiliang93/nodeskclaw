...
DeskClaw 后端服务 -- 人与 AI 共同经营的 API 中枢。驱动组织运转、AI 经营伙伴的部署编排、赛博办公室消息总线、基因能力分发等核心经营逻辑。基于 FastAPI 构建，提供集群管理、实例部署、镜像仓库管理等 API。
...
| `POST /api/v1/workspaces/maintenance/batch-upgrade-instances` | 维护 | 批量升级指定 runtime 的实例镜像版本（`runtime` 必填，支持 `dry_run` 预览和 `with_repair` channel 修复，super_admin） |
...
| 表 | 用途 | 谁写 | 谁读 |
|----|------|------|------|
| `org_memberships` | Portal 组织角色（admin/member） | 邀请/管理员添加 | Portal |
...
Agent 响应以 `delegate:<target>` 或 `escalate:<target>` 开头时，自动构建新 envelope 转发给目标 Agent 或 Human。协作深度限制为组织级配置 `Organization.max_collaboration_depth`（默认 3，范围 1-10，硬上限 20），可通过 Portal 组织信息页实时调整。delegation 链通过 `routing.visited` 追踪已访问节点，`causationid`/`correlationid` 追踪因果链。
...
钉钉收发消息能力由 OpenClaw Channel 插件（`openclaw-channel-dingtalk/`）实现，凭证通过 Channel 配置页面写入实例。
