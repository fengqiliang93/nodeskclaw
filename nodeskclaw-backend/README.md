...
│   │   │   │   └── ingestion/    # 接入层适配器（Portal/Agent/Feishu/DingTalk/Cron）
...
```
入口（Portal/Agent SSE/Feishu/DingTalk/System）
...
指定 Channel（Feishu/DingTalk/...） → SSE → 离线队列（enqueue）。每级失败自动尝试下一级。
...
钉钉 Channel 配置（可选，用于 Platform 层消息投递）：

| 变量 | 说明 |
|------|------|
| `DINGTALK_APP_KEY` | 钉钉应用 App Key（Client ID），用于获取 access_token |
| `DINGTALK_APP_SECRET` | 钉钉应用 App Secret（Client Secret） |
