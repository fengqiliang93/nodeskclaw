...
基于当前集群实测（Host IP: <LAN_IP>）：

- Portal: http://<LAN_IP>:30080
- Employee-01 域名入口: http://test-employee-01-ratszo.<LAN_IP>.nip.io
- Employee-01 NodePort 入口: http://<LAN_IP>:31889
- Employee-02 域名入口: http://test-employee-02.<LAN_IP>.nip.io
- Employee-02 NodePort 入口: http://<LAN_IP>:31947
...
- Ingress host 已同步到当前 LAN IP（如 *.<LAN_IP>.nip.io）。
