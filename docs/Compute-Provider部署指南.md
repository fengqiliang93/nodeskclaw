...
nodeskclaw-backend:
  volumes:
    ...
    - ${NODESKCLAW_DATA_DIR:-$HOME/.nodeskclaw/docker-instances}:${NODESKCLAW_DATA_DIR:-$HOME/.nodeskclaw/docker-instances}
  environment:
    DOCKER_DATA_DIR: ${NODESKCLAW_DATA_DIR:-$HOME/.nodeskclaw/docker-instances}
```
...
| `${NODESKCLAW_DATA_DIR:-...}` | 实例数据持久化目录，宿主机和容器内路径必须一致 |

**Windows (Docker Desktop) 注意事项：**

Windows 上 `$HOME` 可能无法被 Docker Compose 正确展开。需要在项目根目录 `.env` 中显式设置 `NODESKCLAW_DATA_DIR` 为完整的宿主机路径：
...
`DOCKER_DATA_DIR` 不设置时默认为 `~/.nodeskclaw/docker-instances`。
...
4. **数据持久化** — 实例数据挂载到 `{DOCKER_DATA_DIR}/{slug}/data`
...
| 变量 | 必填 | 说明 | 默认值 |
|---|---|---|---|
| `DOCKER_DATA_DIR` | 否 | 后端进程使用的实例数据存储目录 | `~/.nodeskclaw/docker-instances` |
| `NODESKCLAW_DATA_DIR` | 否 | docker-compose.yml 卷挂载路径覆盖（Windows 必填） | `$HOME/.nodeskclaw/docker-instances` |
...
Windows 的 Git Bash / CMD / PowerShell 中 `$HOME` 行为不一致。在项目根目录 `.env` 中设置：
