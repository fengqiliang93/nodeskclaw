...
nodeskclaw-backend:
  volumes:
    - /var/run/docker.sock:/var/run/docker.sock       # Docker daemon 访问
    - $HOME/.nodeskclaw/docker-instances:$HOME/.nodeskclaw/docker-instances  # 实例数据
  environment:
...
| `$HOME/.nodeskclaw/docker-instances` | 实例数据持久化目录，宿主机和容器内路径必须一致 |
...
| 变量 | 必填 | 说明 | 默认值 |
|---|---|---|---|
| `DOCKER_DATA_DIR` | 否 | 实例数据存储目录 | `~/.nodeskclaw/docker-instances` |
