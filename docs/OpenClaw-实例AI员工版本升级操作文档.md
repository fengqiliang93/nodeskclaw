...
docker tag \
  nodesk-center-cn-beijing.cr.volces.com/public/deskclaw-openclaw:v2026.4.22 \
  <LAN_IP>:5000/deskclaw-openclaw:v2026.4.22

# 2) 推送内网仓库
docker push <LAN_IP>:5000/deskclaw-openclaw:v2026.4.22
```
...
curl -s -X PUT "$BASE/api/v1/settings/image_registry" \
  ...
  -d '{"value":"<LAN_IP>:5000/deskclaw-openclaw"}' | jq
```
...
```bash
BASE=http://<portal-host>:30080

TOKEN=$(curl -s "$BASE/api/v1/auth/account-login" \
  -H "Content-Type: application/json" \
  -d '{"account":"<admin-user>","password":"<admin-password>"}' \
...
### 8.3 同 tag 重建后的强制拉取修复

如果某个版本 tag 已经在集群节点上缓存过错误镜像，后续即使重新 push 同名 tag，K8s 默认也不会自动重新拉取。

此时批量升级后还需要补一次 Deployment patch：

```bash
python3 - <<'PY'
import json, subprocess, time

target_image = "<LAN_IP>:5000/deskclaw-openclaw:v2026.5.18"
force_pull_at = str(int(time.time()))
ctx = "default"
...
for item in raw["items"]:
    ...
    if not ns.startswith("nodeskclaw-default-"):
        continue
    ...
    patch = {
        "spec": {
            "template": {
                "metadata": {
                    "annotations": {
                        "nodeskclaw/forcePullAt": force_pull_at,
                        "nodeskclaw/image-tag": "v2026.5.18",
                    }
                },
                "spec": {
                    "initContainers": [
    ...
    subprocess.check_call([
        "kubectl", "--context", ctx, "patch", "deploy", name, "-n", ns,
...
1. 同一版本 tag 曾被错误镜像占用过。
2. Deployment 已显示新 tag，但 Pod 日志仍然表现为旧脚本/旧二进制。
3. canary 正常、批量异常，且排查确认 registry 中镜像 digest 已更新。
...
./build-full.sh --source-path ../openclaw --version 2026.5.7-feishu2 \
  --image <LAN_IP>:5000/deskclaw-openclaw:v2026.5.7-feishu2 --build-only
docker push <LAN_IP>:5000/deskclaw-openclaw:v2026.5.7-feishu2
...
### 12.7 升级后搜索能力丢失或 SearXNG 配置消失

典型现象：

- 实例能启动，但 `tools.web.search` 为空或未启用；
- `plugins.entries.searxng` 在实例 `openclaw.json` 中缺失；
- 升级前可搜索，重启后搜索能力消失。

根因：

- 自定义 entrypoint 如果“清理残留”时直接删除整个 `plugins` 节点，会把合法的 `searxng` 插件配置一起删掉。

修复要求：

1. `openclaw.json.template` 必须内置：
   - `tools.web.search.enabled=true`
   - `tools.web.search.provider=searxng`
   - `plugins.entries.searxng.config.webSearch.baseUrl`
2. backend 的配置写入、runtime adapter、gene install adapter、启动期 reconcile，必须统一调用同一个 `ensure_searxng_web_search()` 修复函数。
3. `docker-entrypoint.sh` 只能精确清理失效的托管 plugin/channel 残留，不能 `delete c.plugins`。

当前现网默认地址：

```text
http://searxng-web.nodeskclaw-staging.svc.cluster.local:8080
```

### 12.8 2026.5.18 全量升级验收口径

推荐先选一台实例做 canary，再全量升级。
...
```bash
cd /path/to/nodeskclaw/nodeskclaw-artifacts
./build-full.sh \
...
2. canary 先只升级 `fengqiliang`，人工确认正常后再全量。
3. 平台验收：
   - `GET /api/v1/instances` 中全部 openclaw 实例为 `running + healthy + ready`
   - 全部实例 `image_version=v2026.5.18`
4. 浏览器自动化验收：
   - 覆盖全部 openclaw AI 实例
   - Control UI 可打开
   - provider 检查通过
5. 本轮结果：45/45 实例升级完成，45/45 浏览器自动化覆盖通过。
