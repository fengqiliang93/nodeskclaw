...
- 部署机：`10.100.15.9`，同时作为 K8s 节点之一，对外提供 Portal、Ingress、NodePort 入口
- 其它 K8s 节点：例如 `10.100.15.7`，实例 Pod 可能调度到任一节点
- 访问机：同一局域网内的其它电脑，通过浏览器访问 Portal 和实例域名
- 可选模型上游机：可与部署机同机，也可以是集群外另一台机器

示例地址：

- 部署机 LAN IP：`10.100.15.9`
- 其它节点 IP：`10.100.15.7`
- 模型上游地址：`http://10.100.15.9:3000/v1`
- Portal 地址：`http://10.100.15.9:30080`

说明：

- 实例 Pod 可以实际运行在 `10.100.15.9` 或 `10.100.15.7`。
- 浏览器访问入口仍以部署机暴露的地址为准，不直接访问 Pod IP。
- 如使用 Ingress 域名，域名应统一指向部署机 LAN IP，例如 `10.100.15.9`。
...
- Server 节点（部署机）：`10.100.15.9`
- Agent 节点：`10.100.15.7`
...
```bash
curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="server --write-kubeconfig-mode 644 --node-ip 10.100.15.9 --advertise-address 10.100.15.9 --flannel-backend host-gw" sh -
```
...
预期此时至少能看到 `10.100.15.9` 这一台节点为 `Ready`。
...
```bash
curl -sfL https://get.k3s.io | K3S_URL=https://10.100.15.9:6443 K3S_TOKEN=<上一步读取的token> INSTALL_K3S_EXEC="agent --node-ip 10.100.15.7" sh -
```
...
10.100.15.9    Ready    control-plane,master   10.100.15.9
10.100.15.7    Ready    <none>                 10.100.15.7
```

如果 `10.100.15.7` 没有 `Ready`，先排查：
...
如需在其它机器使用 `kubectl`，把其中默认的 `127.0.0.1` 改成 `10.100.15.9` 后再分发。
...
- 因此本地离线镜像应导入到每一台可能承载 Pod 的节点，而不只是 `10.100.15.9`。
...
如果 `10.100.15.7` 也会承载实例 Pod，需要在该机器上执行同等导入，或确保其可以从远端仓库拉取相同镜像。
...
bash deploy/lan-access.sh bootstrap \
  ...
  --llm-target-base-url http://10.100.15.9:3000/v1 \
...
- `--lan-ip` 应填写对外统一访问入口 IP；当前场景为部署机 `10.100.15.9`。
- Pod 即使被调度到 `10.100.15.7`，Portal/实例入口仍通过 Service/Ingress 转发，不需要把入口改成 Pod 所在节点。
...
```bash
bash deploy/lan-access.sh sync-instances --lan-ip 10.100.15.9 --ingress-class traefik
```
...
```bash
bash deploy/lan-access.sh verify-services --lan-ip 10.100.15.9
```
...
1. 访问 `http://10.100.15.9:30080`，确认登录页可打开。
2. 使用管理员账号登录（默认 `admin` / `xprecise` 或你自定义的密码）。
...
```bash
bash deploy/lan-access.sh print-kubeconfig --lan-ip 10.100.15.9
```
...
- 若 Pod 在 `10.100.15.7`，但入口是 `10.100.15.9`，这属于正常情况；优先排查 Service、Ingress、NetworkPolicy，而不是直接访问 Pod IP。

2. 访问机打不开 Portal
- 检查部署机防火墙、交换机 ACL、端口开放情况。
- 在访问机执行 `curl http://10.100.15.9:30080` 验证网络层。

3. 实例链接 502
- 实例刚重启或 Ingress 尚未收敛。
- 先执行：`bash deploy/lan-access.sh verify-services --lan-ip 10.100.15.9`。
- 再执行：`bash deploy/lan-access.sh sync-instances --lan-ip 10.100.15.9 --ingress-class traefik`。
...
bash deploy/lan-access.sh bootstrap --lan-ip 10.100.15.9 ...
```
...
8. `NodePort` 地址可见但连接拒绝（如 `10.100.15.9:31549`）
...
```bash
mkdir -p /home/xprecise/nodeskclaw-proxy
cat > /home/xprecise/nodeskclaw-proxy/xprecise-31549.conf <<'EOF'
server {
  listen 31549;
  server_name _;
  location / {
    proxy_pass http://127.0.0.1:80;
    proxy_set_header Host xprecise-x-claw-ms2pka.10.100.15.9.nip.io;
...
docker rm -f xprecise-31549-proxy >/dev/null 2>&1 || true
docker run -d --name xprecise-31549-proxy --restart unless-stopped \
  --network host \
  -v /home/xprecise/nodeskclaw-proxy/xprecise-31549.conf:/etc/nginx/conf.d/default.conf:ro \
  nginx:1.27-alpine
```
...
```bash
# 1) 清理平台与实例
bash deploy/lan-access.sh reset-all --context default --lan-ip 10.100.15.9
...
# 5) 重新初始化
bash deploy/lan-access.sh bootstrap --context default --lan-ip 10.100.15.9 --ingress-class traefik
bash deploy/lan-access.sh sync-instances --context default --lan-ip 10.100.15.9 --ingress-class traefik
bash deploy/lan-access.sh verify-services --context default --lan-ip 10.100.15.9
```
...
```bash
bash deploy/lan-access.sh bootstrap --lan-ip 10.100.15.9 --ingress-class traefik
bash deploy/lan-access.sh sync-instances --lan-ip 10.100.15.9 --ingress-class traefik
bash deploy/lan-access.sh verify-services --lan-ip 10.100.15.9
```
