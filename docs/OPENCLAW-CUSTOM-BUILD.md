...
1. 必须显式使用本地 OpenClaw 源码目录，不要退回 npm 包构建。
2. 必须使用 `build-full.sh`，不要手工改用其它 Dockerfile 流程。
3. 生产升级不要复用旧 tag；如果因为排障必须重建同名 tag，K8s 侧必须同步强制重新拉取。
...
### 同 tag 重建的额外要求
- 如果某个 tag 曾经推送过错误镜像，又被重新构建为同名 tag，K8s 默认 `IfNotPresent` 会继续命中节点旧缓存。
- 发布时至少同步处理以下 3 项：
  1. 主容器 `imagePullPolicy=Always`
  2. init container `imagePullPolicy=Always`
  3. Deployment annotation 增加一次新的强制 rollout 标记（如 `nodeskclaw/forcePullAt`）
- 否则会出现 Deployment 已显示新 tag，但实际 Pod 仍跑旧内容的假象。
