...
- 默认值保持兼容：`http://<llm-host>:3000/v1` + `sk-local-placeholder`
...
bash deploy/lan-access.sh bootstrap \
  ...
  --llm-target-base-url http://<llm-host>:31000/v1 \
...
bash deploy/lan-access.sh ensure-llm-proxy \
  ...
  --llm-target-base-url http://<LAN_IP>:31000/v1 \
  --llm-api-key sk-local-placeholder
```
...
- 实例 `test-openclaw-a2`：可达（http://<LAN_IP>:30333）
- 实例聊天（newapi/deepseek-v3.2）：已验收，返回正确回复
- 模型路由：实例 Pod → 集群内 llm-proxy → 外部 newapi（<LAN_IP>:3000）
- 账号：你显式设置的管理员账号/密码（2026-05-07 验收口径）

## 后端代码级修复（2026-05-07）

`nodeskclaw-backend/app/services/llm_config_service.py` 已修复：

- **问题**：`personal` key_source 会直接将用户配置的外部 baseUrl 写入实例 `openclaw.json`，实例 Pod 无法直连外部地址（<LAN_IP>:3000 在 Pod 网络中不可达）
