...
LLM_TARGET_BASE_URL="${LLM_TARGET_BASE_URL:-http://10.100.15.9:3000/v1}"
...
ADMIN_PASSWORD="${ADMIN_PASSWORD:-xprecise}"
LAN_IP="${LAN_IP:-}"
...
命令选项:
  import-images:
    ...
    --llm-target-base-url <url>     llm-proxy 转发目标（默认: http://10.100.15.9:3000/v1）
  ...
  bootstrap:
    ...
    --admin-password <pwd>          管理员密码（默认: xprecise）
...
示例:
  ...
  ./deploy/lan-access.sh bootstrap --lan-ip 10.100.12.211
  ...
  ./deploy/lan-access.sh print-kubeconfig --lan-ip 10.100.12.211
  ./deploy/lan-access.sh verify-services --lan-ip 10.100.12.211
