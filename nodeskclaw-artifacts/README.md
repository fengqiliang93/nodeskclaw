...
docker build \
  ...
  -t cr-cn-beijing.<CLOUD_VENDOR_DOMAIN>/nodeskclaw/openclaw:v1.0.0 \
  .
```
...
docker push cr-cn-beijing.<CLOUD_VENDOR_DOMAIN>/nodeskclaw/openclaw:v1.0.0
```
...
NoDeskClaw 使用 Nginx Ingress Controller 实现 OpenClaw 实例的子域名自动路由。每个实例部署时自动创建 Ingress 规则，用户通过 `{instance-name}.{base-domain}` 访问对应实例。

### 架构

```
用户浏览器 → *.example.com DNS → 负载均衡器 → K8s Ingress Controller (NodePort 30080/30443) → ClusterIP Service (:18789) → OpenClaw Pod
```
...
- **本目录** -- 被 NoDeskClaw 部署到 K8s 的 OpenClaw 实例镜像
