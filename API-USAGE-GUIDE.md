...
> **网关地址**：`http://<gateway-host>:8080`
> **API Key**：`nd-your-api-key`
> **渠道名称**：`aws`
...
```bash
curl -X POST http://<gateway-host>:8080/default/passthrough \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer nd-your-api-key" \
  -d '{
    "channel": "aws",
...
```bash
curl -X POST http://<gateway-host>:8080/default/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer nd-your-api-key" \
  -d '{
    "model": "us.anthropic.claude-sonnet-4-20250514-v1:0",
...
client = OpenAI(
    api_key="nd-your-api-key",
    base_url="http://<gateway-host>:8080/default/v1"
)
...
| 配置项 | 值 |
|--------|-----|
| API Base URL | `http://<gateway-host>:8080/default/v1` |
| API Key | `nd-your-api-key` |
